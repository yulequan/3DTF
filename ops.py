...
def conv3d_bn_relu(input, output_chn, kernel_size, stride, channel_dim =-1, use_bias=False, training=True, use_bn = True,
                   name='conv'):
    if channel_dim==1:
...
def deconv3d_bn_relu(input, output_chn, kernel_size = 4, stride = 2, channel_dim =-1, use_bias = False, training = True,
                     use_bn = True, name='deconv'):
    if channel_dim==1:
        data_format = 'channels_first'
    else:
        data_format ='channels_last'
    with tf.variable_scope(name):
        ...
        if use_bn:
            ...
            relu = tf.nn.relu(deconv,name='relu')
    return relu
...
# dice loss function
def dice_loss(labels, logits, num_classes = None, smooth=1e-5, loss_collection=tf.GraphKeys.LOSSES, scope=None):
    #labels [batch_size, d0, d1, ... dr]
    #logits [batch_size, d0, d1, ... dr, num_classes]
    ...
    with tf.name_scope(scope):
        ...
        inse = tf.reduce_sum(labels*softmax,axis=axis)
        l = tf.reduce_sum(softmax*softmax,axis=axis)
...
def weighted_softmax_cross_entropy(labels, logits, weighted="median_freq", num_classes= None,loss_collection=tf.GraphKeys.LOSSES,scope=None):
    with tf.name_scope(scope):
        if num_classes is None:
            num_classes = logits.get_shape()[-1]
        labels = tf.one_hot(labels, depth=num_classes)
        axis = range(len(labels.shape)-1)
        epsilon = tf.constant(value=1e-10)

        softmax = tf.nn.softmax(logits+epsilon)+epsilon
        if weighted == 'median_freq':
            freq = tf.reduce_sum(labels, axis=axis) / tf.to_float(tf.reduce_sum(labels))
            median_freq = tf.nn.top_k(freq, num_classes // 2).values[num_classes // 2 - 1]
            weights = median_freq / (freq + epsilon)
        elif weighted =='natural_freq':
            freq = tf.reduce_sum(labels, axis=axis) / tf.to_float(tf.reduce_sum(labels))
            weights = 1.0 - freq

        cross_entropy = -tf.reduce_sum(labels*tf.log(softmax)*weights,axis=-1)
...
def focal_loss(labels,logits, gamma = 2, num_classes = None, loss_collection = tf.GraphKeys.LOSSES,scope=None):
    with tf.name_scope(scope):
        if num_classes is None:
            num_classes = logits.get_shape()[-1]
        labels = tf.one_hot(labels,depth=num_classes)
        ...
        focal_loss = -tf.reduce_sum(labels *tf.pow((1-softmax),gamma)*tf.log(softmax), axis=-1)
        focal_loss_mean = tf.reduce_mean(focal_loss, name='focal_loss')
        tf.add_to_collection(loss_collection, focal_loss_mean)
