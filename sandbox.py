...
def div():
    pair_list = glob('{}/*/*/*_seg.nii.gz'.format("/home/lqyu/server/gpu8/BRATS2017/data/Brats17TrainingData/"))
    pair_list.sort()
    random.shuffle(pair_list)
    train_list = pair_list[1:int(0.8*len(pair_list))]
    val_list = pair_list[int(0.8*len(pair_list)):]

    train_list  = [item.split('/')[-2] for item in train_list]
    val_list    = [item.split('/')[-2] for item in val_list]

    pickle.dump([train_list,val_list],open('/home/lqyu/server/gpu8/BRATS2017/data/Brats17TrainingData/division.pkl','wb'))
