...
def main(_):
    ...
    model_path = "../model/UNet_feat_simple"
    ...
    param_set['labeling_dir'] = '/home/lqyu/server/gpu8/BRATS2017/result/UNet_feat_simple'
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=False)) as sess:
        ...
        if param_set['phase'] == 'train':
            # copy the model
            if os.path.exists(os.path.join(model_path, 'code')):
                os.removedirs(os.path.join(model_path, 'code'))
